...
const Confirmation = () => {
  ...
  const abspath = process.env.REACT_APP_ORDERCONFIRMATION_ABS_PATH;
  const relativepath = process.env.REACT_APP_ORDERCONFIRMATION_REL_PATH;
  ...
  const goBack = () => {
    ...
    let minPath = "bauereg/Orders/Bauer/Order Confirmation";
    const getData = async () => {
      try {
        setLoading(true);
  ...
  const AnalyzeItem = (e) => {
    ...
    const getData = async () => {
      try {
        setError(false);
        setLoading(true);
        const url = `${baseURL}/OrdersConfirmationpdfAnalysis?filepath=${targetpath}`;
        const data = await fetchDataOnly(url, "GET", token);
        console.log(data);
        setLoading(false);
        setIsSuccess(true);
