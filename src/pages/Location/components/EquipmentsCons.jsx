...
import useEqCons from "../controller/eqConsController";
  ...
  if (active && payload && payload.length) {
    return (
      <div className="p-4 bg-white border-1 border-gray-300">
        ...
        <p className="text-[10px] font-[600]">{`Total Quantity: ${payload[0]?.payload?.["Total Quantity"]}`}</p>
        <p className="text-[10px] font-[600]">{`Total Operation Hours: ${payload[0]?.payload?.["Total Operation Hours"]}`}</p>
...
const EquipmentConsCard = ({ title, data, currentSpare }) => {
  const { chartData } = useEqCons({ data, currentSpare });
...
export default EquipmentConsCard;
