...
const VirtualTable = ({
  ...
  const getData = async () => {
    try {
      ...
      let sourceData = tableData?.[sourceTable]?.data
        ? tableData?.[sourceTable]?.data
        : [];
      let currentVT = [];
      ...
      relationShips.map((item) => {
        ...
        currentVT.push(
          ...sourceData?.map((row1) => {
            const match = tableData?.[item?.target]?.data.find(
              (row2) =>
  ...
  if (loading)
    return (
      <div className="relative z-[1] p-1 w-[100vw] h-[100vh]">
