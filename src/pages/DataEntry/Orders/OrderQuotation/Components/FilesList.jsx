...
const FilesList = ({
  ...
  const handleDeleteFile = async (file, fileName) => {
    try {
      setLoading(true);
      const url = `/api/v3/dataEntryOrderQuotationDeleteFiles`;
  ...
  const handleGetReport = async (file, fileName) => {
    ...
    const getData = async () => {
      try {
        setLoading(true);
        const url = `api/v3/dataEntryOrderQuotationAnalyze`;
            ...
            >
              ...
              <div
                ...
                onClick={() => {
                  window
                    .open(
                      `${baseURL}/${process.env.REACT_APP_ORDERQUOTATION_REL_PATH}/${path}/${file?.file}`,
                      "_blank"
                    )
                    .focus();
                }}
