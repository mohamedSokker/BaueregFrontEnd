...
const OrderInvoice_NotFound = () => {
  ...
  const getData = async () => {
    try {
      setLoading(true);
      const url = `/api/v3/OrderInvoice_NotFound`;
  ...
  if (loading)
    return <Spinner message={`Loading OrderInvoice_NotFound Data`} />;
  return (
    <div
    ...
    >
      <React.Fragment>
        <div className="flex flex-row items-center justify-between Header mb-10 ">
          <Header category="" title={"OrderInvoice_NotFound"} />
        </div>
        <GridComponent
...
export default OrderInvoice_NotFound;
