...
const Table = ({ setIsTable, tableData, setTableData, setLoading }) => {
    ...
    try {
      ...
      tableData?.map((item, i) => {
        targetTable.push({
          ...
          InvoiceNo: item.targetInvoiceNo,
          ReferenceNo: item.orderNo,
          OrderNo: item.referenceNo,
          Description: item.desc,
          ItemNo: item.partNo,
          Quantity: item.Quantity,
          Unit: item.unit,
          Total_EURO: item.totalValue,
          ShipmentMode: item.targetShipmentMode,
        });
      });
  ...
  const handleSend = async () => {
    try {
      setLoading(true);
      const url = `/api/v3/dataEntryOrderInvoiceAddOrder`;
      const targetData = [];
      tableData.map((item) => {
        delete item.ID;
