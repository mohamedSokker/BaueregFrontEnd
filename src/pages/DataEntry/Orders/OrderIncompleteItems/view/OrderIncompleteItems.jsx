...
const OrderIncompleteItems = () => {
  ...
  const getData = async () => {
    try {
      setLoading(true);
      const url = `/api/v3/Order_IncompleteItems`;
  ...
  if (loading)
    return <Spinner message={`Loading Order_IncompleteItems Data`} />;
  return (
    <div
    ...
    >
      <React.Fragment>
        <div className="flex flex-row items-center justify-between Header mb-10 ">
          <Header category="" title={"Order_IncompleteItems"} />
        </div>
        <GridComponent
...
export default OrderIncompleteItems;
