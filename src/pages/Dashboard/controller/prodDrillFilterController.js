...
import { prodDrillFilterModel } from "../Model/model";
...
const useFilter = ({ copiedData, setData, count, setCount }) => {
  ...
  useEffect(() => {
    if (!prodDrillFilterModel.checkedItems) {
      let eqs = [];
      let sites = [];
      ...
      const avTrenchData = copiedData.prodDrill.filter((d) =>
        d["# Machine"].startsWith("BG")
      );
      ...
      setItems(prodDrillFilterModel.checkedItems);
      setCount(prodDrillFilterModel.count);
      setFilteredData(prodDrillFilterModel.filteredData);
    }
  }, []);
  ...
  const handleCheck = ({ title, store, index }) => {
    ...
    const filterData = copiedData.prodDrill?.filter(
      (item) =>
