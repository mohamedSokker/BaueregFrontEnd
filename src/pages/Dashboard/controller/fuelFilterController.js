...
import { fuelFilterModel } from "../Model/model";
...
const useFilter = ({ copiedData, setData, count, setCount }) => {
  ...
  useEffect(() => {
    if (!fuelFilterModel.checkedItems) {
      let eqs = [];
      let sites = [];
      ...
      setItems(fuelFilterModel.checkedItems);
      setCount(fuelFilterModel.count);
      setFilteredData(fuelFilterModel.filteredData);
    }
  }, []);
