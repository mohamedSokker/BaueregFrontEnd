...
const ProdTrenchTable = ({ setIsProdTrenchTable, data }) => {
  ...
  const getData = async () => {
    try {
      setTableGrid([]);
      setTableData(data?.prodTrench);
      data?.prodTrench &&
        data?.prodTrench[0] &&
        Object.keys(data?.prodTrench[0]).map((item, i) => {
          setTableGrid((prev) => [
            ...prev,
            {
              field: item,
...
export default ProdTrenchTable;
