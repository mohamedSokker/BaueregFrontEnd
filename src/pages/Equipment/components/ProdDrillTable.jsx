...
const ProdDrillTable = ({ setIsProdDrillTable, data }) => {
  ...
  const getData = async () => {
    try {
      setTableGrid([]);
      setTableData(data?.prodDrill);
      data?.prodDrill &&
        data?.prodDrill[0] &&
        Object.keys(data?.prodDrill[0]).map((item, i) => {
          setTableGrid((prev) => [
            ...prev,
            {
              field: item,
