...
const Child = ({
  ...
  return ReactDOM.createPortal(
    <div
      className=" bg-white  max-h-[200px] overflow-y-scroll text-[10px] border border-gray-400 rounded-[6px]"
      style={{
        width: width,
        top: position.top,
        left: position.left,
    ...
    >
      {childItems?.map((item, idx) => (
        <div
          key={idx}
          className="w-full flex flex-row items-center gap-1 p-1 px-2 hover:cursor-pointer hover:bg-gray-300"
          onClick={() => {
...
const PropertyCard = ({
  ...
  return (
    ...
    >
      ...
      <div
        className="w-full h-[25px] px-2 flex flex-row items-center justify-between bg-gray-400 opacity-60 border-gray-500 border rounded-md relative "
        style={{
      ...
      >
        {isChangeProp ? (
          <form className="w-[100%] text-[10px]" onSubmit={handleSubmit}>
            <input
              type="text"
              id={`changePropInput-${
