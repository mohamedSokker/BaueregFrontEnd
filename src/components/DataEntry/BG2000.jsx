...
const BG2000 = ({ setPerMaintData, setSaved }) => {
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        setPerMaintData({});
        const url = `/readExcel?path=c:/xampp/htdocs/bauereg/PeriodicMaint.xlsx&sheet=BG2000`;
  ...
  // useEffect(() => {
  //   setPerMaintData((prev) => ({ ...prev, BG2000: selectedData }));
  // }, [selectedData]);
  ...
  const handleSave = () => {
    ...
    setPerMaintData((prev) => ({ ...prev, BG2000: newData }));
    setSaved((prev) => ({ ...prev, BG2000: true }));
  };
  return (
    <div className="absolute top-0 left-0 w-full h-full bg-[rgba(0,0,0,0.5)] flex items-center justify-center z-9">
      {loading ? (
        <div className="p-4 mt-20">
          <PageLoading message={`Loading View`} />
        </div>
      ) : (
        <div className="p-4 mt-20 flex flex-nowrap whitespace-nowrap text-white flex-col bg-logoColor rounded-md gap-4 border-1 border-white w-[60%] max-h-[60%] overflow-y-auto">
          <div className="flex justify-center items-center w-full text-orange-500 font-extrabold text-[20px] mt-auto">
            <p>BG2000 Periodic Maintenance Interval</p>
          </div>
          {dataTitles?.map((d, i) => (
            <div
