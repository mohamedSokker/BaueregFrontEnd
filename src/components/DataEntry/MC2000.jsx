...
const MC2000 = ({ setPerMaintData, setSaved }) => {
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        setPerMaintData({});
        const url = `/readExcel?path=c:/xampp/htdocs/bauereg/PeriodicMaint.xlsx&sheet=MC2000`;
  ...
  const handleSave = () => {
    ...
    setPerMaintData((prev) => ({ ...prev, MC2000: newData }));
    setSaved((prev) => ({ ...prev, MC2000: true }));
  };
  return (
      ...
      {loading ? (
        ...
        <div className="p-4 mt-20 flex flex-nowrap whitespace-nowrap text-white flex-col bg-logoColor rounded-md gap-4 border-1 border-white w-[60%] max-h-[60%] overflow-y-auto">
          <div className="flex justify-center items-center w-full text-orange-500 font-extrabold text-[20px] mt-auto">
            <p>MC2000 Periodic Maintenance Interval</p>
          </div>
          {dataTitles?.map((d, i) => (
            <div
