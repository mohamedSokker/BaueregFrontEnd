...
const BG250 = ({ setPerMaintData, setSaved }) => {
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        setPerMaintData({});
        const url = `/readExcel?path=c:/xampp/htdocs/bauereg/PeriodicMaint.xlsx&sheet=BG250`;
  ...
  // useEffect(() => {
  //   setPerMaintData((prev) => ({ ...prev, BG250: selectedData }));
  // }, [selectedData]);
  ...
  const handleSave = () => {
    ...
    setPerMaintData((prev) => ({ ...prev, BG250: newData }));
      ...
      {loading ? (
        ...
        <div className="p-4 flex flex-nowrap whitespace-nowrap text-white flex-col bg-logoColor rounded-md gap-4 border-1 border-white w-[60%] max-h-[60%] overflow-y-auto">
          <div className="flex justify-center items-center w-full text-orange-500 font-extrabold text-[20px] mt-auto">
            <p>BG250 Periodic Maintenance Interval</p>
          </div>
          {dataTitles?.map((d, i) => (
            <div
